...
// Includes both counts-per-interval, counts-per-minute, and a pulse width histogram.  The
// pulse width histogram is included as pulse energy is supposed to correllate with spectral 
// energy -- this is currently just a test, though.
// Peter Jansen, September/2014. 
// -------------------------------------------------------------------------------------------------------------
// This library is interrupt driven, so most everything happens behind the scenes.  It takes a few lines of code to initialize: 
//  // Global
//  SensorBuffer sbRad(100);
//  SensorRadiation sensorRadiation(&sbRad);
//  // In setup(): Initialize radiation sensor
//  setupRadiationISR(&sensorRadiation);     // MUST be called before begin()
//  sensorRadiation.begin();
//  // In loop(): 
//  sensorRadiation.debugPrint();
// There are also methods to control the integration (sample) time, to calculate the counts per minute, and to
// store the pulse width histogram into a SensorBuffer storage class.
// -------------------------------------------------------------------------------------------------------------
...
//  Serial.println ("**** OUT");
  ...
  if (spectralBins[binNum] < 10000) {   // Max out before overflow errors get us
